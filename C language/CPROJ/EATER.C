...
bug ( int *r, int *c, int *dir, unsigned char *ch )
{
	...
	/* in each if statement the 1st condition checks whether the movement in that direction would move the bug closer to the Eater, whereas the 2nd condition checks if the maze permits a movement in that direction */
		...
		switch ( *dir )
		{
			...
			case LEFT :
				...
				if ( maze[*r][*c - 1] == 1 )
				{
					...
					if ( temp == 2 )
					{
						/* if path exists to the right of bug */
						if ( maze[*r][*c + 1] == 1 )
							*dir = RIGHT ;
						else
						{
					...
					{
						/* if there is no bug to the left of the bug being considered, move the bug left */
						movebugleft ( r, c, ch ) ;
						flag = 1 ;
					}
				}
				...
				{
					/* since there is no path in the LEFT direction, try another path */
					*dir = random ( 4 ) ;
				}
